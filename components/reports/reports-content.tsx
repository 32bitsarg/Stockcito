...
export function ReportsContent() {
    const { data: summary, isOfflineData, dataUpdatedAt, isLoading } = useOfflineData(
        ['reportsSummary'],
        getReportsSummary
    )
    ...
    if (!summary) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
                <p className="text-sm text-zinc-500">No se pudieron cargar los reportes. Verifica tu conexión.</p>
            </div>
        )
    }
    ...
    return (
        <div className="pb-10">
            <PageHeader
                ...
                subtitle={`Análisis consolidado del período: ${format(new Date(summary.period.startDate), "dd MMM", { locale: es })} — ${format(new Date(summary.period.endDate), "dd MMM yyyy", { locale: es })}`}
            >
                        ...
                        <Card className="border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shadow-sm overflow-hidden h-full">
                            ...
                            <CardContent>
                                ...
                                <div className="space-y-2">
                                    ...
                                    {summary.clients.topClients.slice(0, 3).map((client: any, index: number) => (
